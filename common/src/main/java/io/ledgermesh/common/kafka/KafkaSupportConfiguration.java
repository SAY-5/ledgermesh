...
/**
 * Topics (with their dead letter and parked shadows) are created on startup by whichever service
 * comes up first. A failing record is retried in place with exponential backoff, which holds the
 * partition while a dependency (database, broker) recovers; once the attempts are used up the
 * record is published to {@code <topic>.dlq} with the original coordinates and the exception in
 * headers, and the partition moves on. Structurally broken payloads skip the retries and go
 * straight to the dead letter topic. Every failed delivery is counted per topic, so the attempts a
 * record burned before it was dead lettered are visible.
 */
@Configuration
public class KafkaSupportConfiguration {
  ...
  public KafkaAdmin.NewTopics ledgermeshTopics() {
    NewTopic[] topics = new NewTopic[Topics.ALL.length * 3];
    for (int i = 0; i < Topics.ALL.length; i++) {
      topics[3 * i] = TopicBuilder.name(Topics.ALL[i]).partitions(3).replicas(1).build();
      topics[3 * i + 1] =
          TopicBuilder.name(Topics.dlq(Topics.ALL[i])).partitions(1).replicas(1).build();
      topics[3 * i + 2] =
          TopicBuilder.name(Topics.parked(Topics.ALL[i])).partitions(1).replicas(1).build();
    }
